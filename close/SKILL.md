...
2. **Uncommitted work check — HARD BLOCK:**
   ...
   - **Daemon-synced portfolio snapshot:** `05-Resources/Portfolio.md` in the BASECAMP repo (its `AUTO:*` fenced blocks — account summary + owned table — are rewritten by `portfolio_sync.py`'s LaunchAgent every 15 min with a live IBKR price/NLV/timestamp refresh, per the file's own `canonical_writer` frontmatter). Whole-file exclusion, same species as the daemon-and-operator-mixed PERSONAL.md above: git path-matching can't drop only the fenced hunks. This is safe because the owned-position data is *separately* guarded against IBKR drift by Step 1a (HARD BLOCK) — so excluding it here drops only cosmetic 15-min price churn, never unverified position state. The two checks are complementary, not redundant.
